...
router.post("/", async (req, res) => {
    try {
        ...
        const Card_Number = Math.floor(1000000000000000 + Math.random() * 9000000000000000).toString();
...
router.get("/byAccount/:Id_Account", async (req, res) => {
    try {
        const { Id_Account } = req.params;

        const account = await Account.findOne({ Id_Account });
        if (!account) {
            return res.status(404).json({ error: "Account not found" });
        }
